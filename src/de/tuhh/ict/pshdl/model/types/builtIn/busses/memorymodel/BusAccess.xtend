		...
		for (Row row:rows) {
			...
			if (!checkedRows.contains(row.name)){
				...
				if (row.hasWriteDefs)
						res=res+'''
							// Setter
							int set«row.name.toFirstUpper»Direct(uint32_t *base, int index«FOR Definition definition:row.writeDefs»«getParameter(row,definition, false)»«ENDFOR»);
							int set«row.name.toFirstUpper»(uint32_t *base, int index, «row.name»_t *newVal);
							'''			
				res=res+'''
					//Getter
					int get«row.name.toFirstUpper»Direct(uint32_t *base, int index«FOR Definition definition:row.allDefs»«getParameter(row,definition, true)»«ENDFOR»);
...
'''
int set«row.name.toFirstUpper»Direct(uint32_t *base, int index«FOR Definition d:row.writeDefs»«row.getParameter(d, false)»«ENDFOR»){
...
'''
	...
	def getBusType(Definition d)
		'''bus_«d.type.toString.toLowerCase»«MemoryModel::getSize(d)»_t'''
