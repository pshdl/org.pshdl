...
public class TypeExtension {
		...
		while (iter.hasNext()) {
			{
				...
				if ((tWidth == null))
					return Optional.<HDLType> absent();
				final HDLArithOp _add = HDLArithOp.add(width, tWidth);
				width = _add;
				final HDLExpression _simplifyWidth = HDLPrimitives.simplifyWidth(cat, width, null);
	...
	protected Optional<? extends HDLType> _determineType(final HDLFunctionCall call) {
		...
		if ((returnType.isPresent() && (returnType.get() instanceof HDLPrimitive.AnyPrimitive))) {
			...
			final boolean _isPresent_1 = specified.isPresent();
			if (_isPresent_1)
				return specified;
		}
		return returnType;
	...
	protected Optional<? extends HDLType> _determineType(final HDLLiteral lit) {
		...
		final boolean _isNegative_1 = lit.isNegative();
		if (_isNegative_1) {
			final HDLPrimitive _integer = HDLPrimitive.getInteger();
			return Optional.<HDLPrimitive> of(_integer);
		}
	...
	protected Optional<? extends HDLType> _determineType(final HDLVariableRef ref) {
		...
		while (iter.hasNext()) {
			{
				...
				final HDLArithOp _add = HDLArithOp.add(width, _width_1);
				width = _add;
				final HDLExpression _simplifyWidth = HDLPrimitives.simplifyWidth(ref, width, null);
				width = _simplifyWidth;
			}
		}
		...
		if (_not)
			return Optional.<HDLType> absent();
		final HDLVariable _get_1 = hVar.get();
