...
public class RangeExtension {
	...
	public static Optional<Range<BigInteger>> rangeOf(final HDLExpression obj, final HDLEvaluationContext context) {
		...
		final boolean _tripleEquals_1 = (range == null);
		if (_tripleEquals_1) {
	...
	protected Optional<Range<BigInteger>> _determineRange(final HDLLiteral obj, final HDLEvaluationContext context) {
		...
		final boolean _tripleEquals = (bigVal == null);
		if (_tripleEquals)
			return Optional.<Range<BigInteger>> absent();
	...
	protected Optional<Range<BigInteger>> _determineRange(final HDLVariableRef obj, final HDLEvaluationContext context) {
		...
		final boolean _tripleNotEquals = (container != null);
		if (_tripleNotEquals) {
			...
			for (final HDLRange r_1 : _bits_1) {
				{
					...
					if (_not_1) {
						...
						final boolean _tripleNotEquals_1 = (bitWidth != null);
		...
		boolean _and = false;
		final boolean _isPresent_5 = type.isPresent();
		if (!_isPresent_5) {
			_and = false;
		} else {
	...
	protected Optional<Range<BigInteger>> _determineRange(final HDLShiftOp obj, final HDLEvaluationContext context) {
		...
		boolean _or = false;
		final boolean _hasLowerBound = lrVal.hasLowerBound();
		final boolean _not_1 = (!_hasLowerBound);
		if (_not_1) {
			_or = true;
		} else {
			final boolean _hasUpperBound = lrVal.hasUpperBound();
			final boolean _not_2 = (!_hasUpperBound);
			_or = _not_2;
		}
		if (_or)
			return Optional.<Range<BigInteger>> absent();
		...
		final boolean _not_3 = (!_isPresent_1);
		if (_not_3)
			return Optional.<Range<BigInteger>> absent();
		final Range<BigInteger> rrVal = rightRange.get();
		boolean _or_1 = false;
		final boolean _hasLowerBound_1 = rrVal.hasLowerBound();
		final boolean _not_4 = (!_hasLowerBound_1);
		if (_not_4) {
			_or_1 = true;
		} else {
			final boolean _hasUpperBound_1 = rrVal.hasUpperBound();
			final boolean _not_5 = (!_hasUpperBound_1);
			_or_1 = _not_5;
		}
		if (_or_1)
			return Optional.<Range<BigInteger>> absent();
	...
	protected Optional<Range<BigInteger>> _determineRange(final HDLBitOp obj, final HDLEvaluationContext context) {
		...
		if (!_matched) {
			boolean _or_2 = false;
			final boolean _equals = Objects.equal(type, HDLBitOp.HDLBitOpType.OR);
			if (_equals) {
				_or_2 = true;
			} else {
				final boolean _equals_1 = Objects.equal(type, HDLBitOp.HDLBitOpType.XOR);
				_or_2 = _equals_1;
			}
			if (_or_2) {
				_matched = true;
		...
		if (!_matched) {
			boolean _or_3 = false;
			final boolean _equals_2 = Objects.equal(type, HDLBitOp.HDLBitOpType.LOGI_AND);
			if (_equals_2) {
				_or_3 = true;
			} else {
				final boolean _equals_3 = Objects.equal(type, HDLBitOp.HDLBitOpType.LOGI_OR);
				_or_3 = _equals_3;
			}
			if (_or_3) {
				_matched = true;
	...
	protected Optional<Range<BigInteger>> _determineRange(final HDLArithOp obj, final HDLEvaluationContext context) {
		...
		if (_type != null) {
			...
			case DIV:
				...
				final boolean _equals = _lowerEndpoint_4.equals(BigInteger.ZERO);
				if (_equals) {
					_or_2 = true;
				} else {
					final BigInteger _upperEndpoint_4 = rrVal.upperEndpoint();
					final boolean _equals_1 = _upperEndpoint_4.equals(BigInteger.ZERO);
					_or_2 = _equals_1;
				}
				if (_or_2) {
					obj.<IHDLObject> addMeta(ProblemDescription.SOURCE, obj);
				...
				boolean _or_3 = false;
				final BigInteger _lowerEndpoint_5 = rrVal.lowerEndpoint();
				final int _signum = _lowerEndpoint_5.signum();
				final BigInteger _upperEndpoint_5 = rrVal.upperEndpoint();
				final int _signum_1 = _upperEndpoint_5.signum();
				final int _multiply = (_signum * _signum_1);
				final boolean _lessThan = (_multiply < 0);
				if (_lessThan) {
					_or_3 = true;
				} else {
					final BigInteger _upperEndpoint_6 = rrVal.upperEndpoint();
					final int _signum_2 = _upperEndpoint_6.signum();
					final boolean _equals_2 = (_signum_2 == 0);
					_or_3 = _equals_2;
				}
				if (_or_3) {
					obj.<IHDLObject> addMeta(ProblemDescription.SOURCE, obj);
				...
				final BigInteger _lowerEndpoint_6 = rrVal.lowerEndpoint();
				final BigDecimal _bigDecimal = new BigDecimal(_lowerEndpoint_6);
				final BigDecimal _divide = BigDecimal.ONE.divide(_bigDecimal);
				final BigInteger _upperEndpoint_7 = rrVal.upperEndpoint();
				final BigDecimal _bigDecimal_1 = new BigDecimal(_upperEndpoint_7);
				...
				final BigDecimal _bigDecimal_2 = new BigDecimal(_lowerEndpoint_7);
				...
				final BigDecimal _bigDecimal_3 = new BigDecimal(_lowerEndpoint_9);
				final BigDecimal _upperEndpoint_8 = mulRange.upperEndpoint();
				final BigDecimal ft = _bigDecimal_3.multiply(_upperEndpoint_8);
				final BigInteger _upperEndpoint_9 = lrVal.upperEndpoint();
				final BigDecimal _bigDecimal_4 = new BigDecimal(_upperEndpoint_9);
				final BigDecimal _lowerEndpoint_10 = mulRange.lowerEndpoint();
				final BigDecimal tf = _bigDecimal_4.multiply(_lowerEndpoint_10);
				final BigInteger _upperEndpoint_10 = lrVal.upperEndpoint();
				final BigDecimal _bigDecimal_5 = new BigDecimal(_upperEndpoint_10);
				final BigDecimal _upperEndpoint_11 = mulRange.upperEndpoint();
			...
			case MUL:
				...
				final BigInteger _upperEndpoint_13 = lrVal.upperEndpoint();
				final BigInteger _lowerEndpoint_14 = rrVal.lowerEndpoint();
				final BigInteger tf_1 = _upperEndpoint_13.multiply(_lowerEndpoint_14);
				final BigInteger _upperEndpoint_14 = lrVal.upperEndpoint();
				final BigInteger _upperEndpoint_15 = rrVal.upperEndpoint();
			...
			case MOD:
				...
				final BigInteger _upperEndpoint_16 = rrVal.upperEndpoint();
			...
			case POW:
				...
				final int _intValue = _lowerEndpoint_16.intValue();
				final BigInteger ff_2 = _lowerEndpoint_15.pow(_intValue);
				final BigInteger _lowerEndpoint_17 = lrVal.lowerEndpoint();
				final BigInteger _upperEndpoint_17 = rrVal.upperEndpoint();
				final int _intValue_1 = _upperEndpoint_17.intValue();
				final BigInteger ft_2 = _lowerEndpoint_17.pow(_intValue_1);
				final BigInteger _upperEndpoint_18 = lrVal.upperEndpoint();
				final BigInteger _lowerEndpoint_18 = rrVal.lowerEndpoint();
				final int _intValue_2 = _lowerEndpoint_18.intValue();
				final BigInteger tf_2 = _upperEndpoint_18.pow(_intValue_2);
				final BigInteger _upperEndpoint_19 = lrVal.upperEndpoint();
				final BigInteger _upperEndpoint_20 = rrVal.upperEndpoint();
				final int _intValue_3 = _upperEndpoint_20.intValue();
		...
		if (_type != null) {
			...
			case CAST:
				...
				if ((type instanceof HDLPrimitive)) {
					...
					final boolean _isPresent_1 = right.isPresent();
					final boolean _not_1 = (!_isPresent_1);
					if (_not_1)
						return Optional.<Range<BigInteger>> absent();
					final HDLPrimitive.HDLPrimitiveType _type_1 = ((HDLPrimitive) type).getType();
					final boolean _equals = Objects.equal(_type_1, HDLPrimitive.HDLPrimitiveType.INTEGER);
					if (_equals) {
						final Range<BigInteger> _intRange = HDLPrimitives.intRange(BigInteger.valueOf(32L));
						final Range<BigInteger> _get = right.get();
						final Range<BigInteger> _intersection = _intRange.intersection(_get);
						return Optional.<Range<BigInteger>> of(_intersection);
					}
					final HDLPrimitive.HDLPrimitiveType _type_2 = ((HDLPrimitive) type).getType();
					final boolean _equals_1 = Objects.equal(_type_2, HDLPrimitive.HDLPrimitiveType.NATURAL);
					if (_equals_1) {
						final Range<BigInteger> _uintRange = HDLPrimitives.uintRange(BigInteger.valueOf(32L));
						final Range<BigInteger> _get_1 = right.get();
						final Range<BigInteger> _intersection_1 = _uintRange.intersection(_get_1);
						return Optional.<Range<BigInteger>> of(_intersection_1);
					}
					final HDLPrimitive.HDLPrimitiveType _type_3 = ((HDLPrimitive) type).getType();
					final boolean _equals_2 = Objects.equal(_type_3, HDLPrimitive.HDLPrimitiveType.BIT);
					if (_equals_2) {
						final Range<BigInteger> _createRange = RangeTool.<BigInteger> createRange(BigInteger.ZERO, BigInteger.ONE);
						final Range<BigInteger> _get_2 = right.get();
						final Range<BigInteger> _intersection_2 = _createRange.intersection(_get_2);
						return Optional.<Range<BigInteger>> of(_intersection_2);
					}
					final boolean _isPresent_2 = castRange.isPresent();
					final boolean _not_2 = (!_isPresent_2);
					if (_not_2)
						return Optional.<Range<BigInteger>> absent();
					final Range<BigInteger> _get_3 = castRange.get();
					final Range<BigInteger> _get_4 = right.get();
					final Range<BigInteger> _intersection_3 = _get_3.intersection(_get_4);
			...
			case ARITH_NEG:
				...
				final BigInteger _upperEndpoint = _get_5.upperEndpoint();
				final BigInteger _negate = _upperEndpoint.negate();
				final Range<BigInteger> _get_6 = right.get();
				final BigInteger _lowerEndpoint = _get_6.lowerEndpoint();
			...
			case BIT_NEG:
				...
				final BigInteger _upperEndpoint_1 = _get_7.upperEndpoint();
