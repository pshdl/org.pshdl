...
/**
 * The ScopingExtension allows the resolution of Enums, Interface, Variables by
 * name
 */
...
public class ScopingExtension {
	...
	public Optional<HDLVariable> resolveVariableDefault(final IHDLObject obj, final HDLQualifiedName hVar) {
		...
		if (_tripleEquals) {
			...
			final boolean _tripleEquals_1 = (_container == null);
			_or = _tripleEquals_1;
		}
		if (_or)
	...
	public Optional<HDLFunction> resolveFunction(final AbstractHDLFunctionCall obj, final HDLQualifiedName hVar) {
		final IHDLObject _container = obj.getContainer();
		final boolean _tripleEquals = (_container == null);
		if (_tripleEquals)
		...
		boolean _or = false;
		final boolean _isPresent = candidates.isPresent();
		final boolean _not = (!_isPresent);
		if (_not) {
			_or = true;
		} else {
			final Iterable<HDLFunction> _get = candidates.get();
			final boolean _isEmpty = IterableExtensions.isEmpty(_get);
			_or = _isEmpty;
		}
		if (_or)
			return Optional.<HDLFunction> absent();
		final Iterable<HDLFunction> _get_1 = candidates.get();
		final SortedSet<HDLFunctions.FunctionScore> scored = HDLFunctions.scoreList(_get_1, ((HDLFunctionCall) obj));
		final boolean _isNullOrEmpty = IterableExtensions.isNullOrEmpty(scored);
		if (_isNullOrEmpty)
			return Optional.<HDLFunction> absent();
		final HDLFunctions.FunctionScore _first = scored.first();
	...
	private HDLResolver resolver(final IHDLObject statement, final boolean descent) {
		...
		final boolean _tripleEquals = (resolver == null);
		if (_tripleEquals) {
			final HDLResolver _hDLResolver = new HDLResolver(statement, descent);
	...
	protected Optional<? extends HDLType> _resolveType(final HDLStatement obj, final HDLQualifiedName hVar) {
		final HDLResolver _resolver = this.resolver(obj, true);
	...
	protected List<HDLEnumDeclaration> _doGetEnumDeclarations(final HDLIfStatement obj) {
		...
		final List<HDLEnumDeclaration> _allEnumDeclarations = HDLResolver.getallEnumDeclarations(_thenDo);
		res.addAll(_allEnumDeclarations);
		final ArrayList<HDLStatement> _elseDo = obj.getElseDo();
		final List<HDLEnumDeclaration> _allEnumDeclarations_1 = HDLResolver.getallEnumDeclarations(_elseDo);
	...
	protected List<HDLInterface> _doGetInterfaceDeclarations(final HDLIfStatement obj) {
		...
		final ArrayList<HDLStatement> _thenDo = obj.getThenDo();
		final List<HDLInterface> _allInterfaceDeclarations = HDLResolver.getallInterfaceDeclarations(_thenDo);
		res.addAll(_allInterfaceDeclarations);
		final ArrayList<HDLStatement> _elseDo = obj.getElseDo();
		final List<HDLInterface> _allInterfaceDeclarations_1 = HDLResolver.getallInterfaceDeclarations(_elseDo);
	...
	protected List<HDLVariable> _doGetVariables(final HDLIfStatement obj) {
		...
		final ArrayList<HDLStatement> _thenDo = obj.getThenDo();
		final List<HDLVariable> _allVariableDeclarations = HDLResolver.getallVariableDeclarations(_thenDo);
		res.addAll(_allVariableDeclarations);
		final ArrayList<HDLStatement> _elseDo = obj.getElseDo();
		final List<HDLVariable> _allVariableDeclarations_1 = HDLResolver.getallVariableDeclarations(_elseDo);
	...
	protected List<HDLInterface> _doGetInterfaceDeclarations(final HDLDirectGeneration gen) {
		final HDLInterface _hIf = gen.getHIf();
		final boolean _tripleEquals = (_hIf == null);
		if (_tripleEquals)
			return Collections.<HDLInterface> emptyList();
		final HDLInterface _hIf_1 = gen.getHIf();
	...
	protected List<HDLVariable> _doGetVariables(final HDLForLoop obj) {
		final List<HDLVariable> res = new LinkedList<HDLVariable>();
		final ArrayList<HDLStatement> _dos = obj.getDos();
		final List<HDLVariable> _allVariableDeclarations = HDLResolver.getallVariableDeclarations(_dos);
		res.addAll(_allVariableDeclarations);
	...
	protected Optional<HDLEnum> _resolveEnum(final HDLAssignment obj, final HDLQualifiedName hEnum) {
		...
		final boolean _tripleEquals = (_container == null);
		if (_tripleEquals) {
			final Problem _problem = new Problem(ErrorCode.UNRESOLVED_ENUM, obj, ("for hEnum:" + hEnum));
	...
	protected Optional<HDLInterface> _resolveInterface(final HDLAssignment obj, final HDLQualifiedName hIf) {
		...
		final boolean _tripleEquals = (_container == null);
		if (_tripleEquals) {
			final Problem _problem = new Problem(ErrorCode.UNRESOLVED_INTERFACE, obj, ("for interface:" + hIf));
			throw new HDLProblemException(_problem);
		}
		final IHDLObject _container_1 = obj.getContainer();
		return this.resolveInterface(_container_1, hIf);
	...
	protected Optional<? extends HDLType> _resolveType(final HDLAssignment obj, final HDLQualifiedName hVar) {
		...
		final boolean _tripleEquals = (_container == null);
		if (_tripleEquals) {
			final Problem _problem = new Problem(ErrorCode.UNRESOLVED_TYPE, obj, ("for type:" + hVar));
			throw new HDLProblemException(_problem);
		}
		final IHDLObject _container_1 = obj.getContainer();
	...
	protected Optional<HDLVariable> _resolveVariable(final HDLAssignment obj, final HDLQualifiedName hVar) {
		...
		final boolean _tripleEquals = (_container == null);
		if (_tripleEquals) {
			final Problem _problem = new Problem(ErrorCode.UNRESOLVED_VARIABLE, obj, ("for hVariable:" + hVar));
			throw new HDLProblemException(_problem);
		}
		final IHDLObject _container_1 = obj.getContainer();
		return this.resolveVariable(_container_1, hVar);
	...
	protected List<HDLInterface> _doGetInterfaceDeclarations(final HDLInterfaceDeclaration obj) {
		final HDLInterface _hIf = obj.getHIf();
	...
	protected List<HDLVariable> _doGetVariables(final HDLInterfaceInstantiation obj) {
		final HDLVariable _var = obj.getVar();
		return Collections.<HDLVariable> singletonList(_var);
	}
	...
	protected List<HDLEnumDeclaration> _doGetEnumDeclarations(final HDLSwitchStatement obj) {
		...
		for (final HDLSwitchCaseStatement c : _cases) {
			final List<HDLEnumDeclaration> _doGetEnumDeclarations = this.doGetEnumDeclarations(c);
	...
	protected List<HDLInterface> _doGetInterfaceDeclarations(final HDLSwitchStatement obj) {
		...
		final ArrayList<HDLSwitchCaseStatement> _cases = obj.getCases();
		for (final HDLSwitchCaseStatement c : _cases) {
			final List<HDLInterface> _doGetInterfaceDeclarations = this.doGetInterfaceDeclarations(c);
	...
	protected List<HDLVariable> _doGetVariables(final HDLSwitchStatement obj) {
		...
		final ArrayList<HDLSwitchCaseStatement> _cases = obj.getCases();
		for (final HDLSwitchCaseStatement c : _cases) {
			final List<HDLVariable> _doGetVariables = this.doGetVariables(c);
	...
	protected Optional<Iterable<HDLFunction>> _resolveFunctionName(final HDLPackage obj, final HDLQualifiedName hFunc) {
		...
		if (_tripleEquals) {
			final String _libURI = obj.getLibURI();
			final HDLLibrary _library = HDLLibrary.getLibrary(_libURI);
			library = _library;
		}
		final String _pkg = obj.getPkg();
		final String _plus = (_pkg + ".*");
		final Iterable<String> _asList = HDLObject.<String> asList(_plus);
	...
	protected Optional<? extends HDLType> _resolveType(final HDLPackage obj, final HDLQualifiedName type) {
		HDLLibrary library = obj.getLibrary();
		final HDLLibrary _library = obj.getLibrary();
		final boolean _tripleEquals = (_library == null);
		if (_tripleEquals) {
			final String _libURI = obj.getLibURI();
			final HDLLibrary _library_1 = HDLLibrary.getLibrary(_libURI);
			library = _library_1;
		}
		final String _pkg = obj.getPkg();
		final String _plus = (_pkg + ".*");
	...
	protected Optional<HDLEnum> _resolveEnum(final HDLUnit obj, final HDLQualifiedName hEnum) {
		final HDLResolver _resolver = this.resolver(obj, false);
		final Optional<HDLEnum> resolveEnum = _resolver.resolveEnum(hEnum);
		final boolean _isPresent = resolveEnum.isPresent();
		if (_isPresent)
			return resolveEnum;
		final Optional<? extends HDLType> res = this.resolveType(obj, hEnum);
		boolean _and = false;
		final boolean _isPresent_1 = res.isPresent();
		if (!_isPresent_1) {
			_and = false;
		} else {
			final HDLType _get = res.get();
	...
	protected Optional<Iterable<HDLFunction>> _resolveFunctionName(final HDLUnit obj, final HDLQualifiedName hFunc) {
		...
		final HDLQualifiedName _skipLast = _fullNameOf.skipLast(1);
		final HDLQualifiedName _append = _skipLast.append("*");
	...
	protected Optional<HDLInterface> _resolveInterface(final HDLUnit obj, final HDLQualifiedName hIf) {
		...
		final boolean _isPresent = resolveInterface.isPresent();
		if (_isPresent)
	...
	protected Optional<HDLVariable> _resolveVariable(final HDLUnit obj, final HDLQualifiedName hVar) {
		...
		final boolean _isPresent = hdlVariable.isPresent();
		if (_isPresent)
			return hdlVariable;
		HDLLibrary library = obj.getLibrary();
		final HDLLibrary _library = obj.getLibrary();
	...
	protected List<HDLEnumDeclaration> _doGetEnumDeclarations(final HDLUnit obj) {
		...
		final ArrayList<HDLStatement> _statements = obj.getStatements();
		final List<HDLEnumDeclaration> _allEnumDeclarations = HDLResolver.getallEnumDeclarations(_statements);
		res.addAll(_allEnumDeclarations);
		return res;
	}
	...
	protected List<HDLInterface> _doGetInterfaceDeclarations(final HDLUnit obj) {
		...
		final ArrayList<HDLStatement> _statements = obj.getStatements();
		final List<HDLInterface> _allInterfaceDeclarations = HDLResolver.getallInterfaceDeclarations(_statements);
		res.addAll(_allInterfaceDeclarations);
		return res;
	}
	...
	protected List<HDLVariable> _doGetVariables(final HDLUnit obj) {
		...
		final ArrayList<HDLStatement> _statements = obj.getStatements();
		final List<HDLVariable> _allVariableDeclarations = HDLResolver.getallVariableDeclarations(_statements);
		res.addAll(_allVariableDeclarations);
		return res;
	}

	protected Optional<HDLVariable> _resolveVariable(final HDLInterface hIf, final HDLQualifiedName hVar) {
		final String _lastSegment = hVar.getLastSegment();
		final HDLVariable resolved = ScopingExtension.getVariable(hIf, _lastSegment);
		final boolean _tripleNotEquals = (resolved != null);
		if (_tripleNotEquals) {
			if ((hVar.length == 1))
				return Optional.<HDLVariable> of(resolved);
			final HDLQualifiedName _fullNameOf = FullNameExtension.fullNameOf(hIf);
			final HDLQualifiedName _skipLast = hVar.skipLast(1);
			final boolean _equals = _fullNameOf.equals(_skipLast);
			if (_equals)
				return Optional.<HDLVariable> of(resolved);
		}
	...
	private static HDLVariable getVariable(final HDLInterface hIf, final String lastSegment) {
		final HDLQuery.Source<HDLVariable> _select = HDLQuery.<HDLVariable> select(HDLVariable.class);
		final HDLQuery.Selector<HDLVariable> _from = _select.from(hIf);
		final HDLQuery.FieldSelector<HDLVariable, String> _where = _from.<String> where(HDLVariable.fName);
		final HDLQuery.Result<HDLVariable, String> _lastSegmentIs = _where.lastSegmentIs(lastSegment);
	...
	protected Optional<HDLVariable> _resolveVariable(final HDLEnum hEnum, final HDLQualifiedName hVar) {
		...
		final HDLQualifiedName _fullNameOf = FullNameExtension.fullNameOf(hEnum);
		final HDLQualifiedName _skipLast = hVar.skipLast(1);
		final boolean _equals = _fullNameOf.equals(_skipLast);
		if (_equals) {
			final String _lastSegment_1 = hVar.getLastSegment();
	...
	public static Optional<HDLVariable> getVariable(final HDLEnum hEnum, final String lastSegment) {
		...
		for (final HDLVariable hVar : _enums) {
			...
			final boolean _equals = _name.equals(lastSegment);
			if (_equals)
	...
	public Optional<HDLVariable> resolveVariable(final IHDLObject hEnum, final HDLQualifiedName hVar) {
		if (hEnum instanceof HDLEnum)
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object> asList(hEnum, hVar).toString());
	}
	...
	public Optional<Iterable<HDLFunction>> resolveFunctionName(final IHDLObject obj, final HDLQualifiedName hFunc) {
		if (obj instanceof HDLPackage)
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object> asList(obj, hFunc).toString());
	}
	...
	public Optional<HDLEnum> resolveEnum(final IHDLObject obj, final HDLQualifiedName hEnum) {
		if (obj instanceof HDLAssignment)
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object> asList(obj, hEnum).toString());
	}
	...
	public Optional<? extends HDLType> resolveType(final IHDLObject obj, final HDLQualifiedName hVar) {
		if (obj instanceof HDLAssignment)
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object> asList(obj, hVar).toString());
	}
	...
	public Optional<HDLInterface> resolveInterface(final IHDLObject obj, final HDLQualifiedName hIf) {
		if (obj instanceof HDLAssignment)
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object> asList(obj, hIf).toString());
	}
