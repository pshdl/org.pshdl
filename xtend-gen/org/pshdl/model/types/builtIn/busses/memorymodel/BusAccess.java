...
public class BusAccess {
	...
	public CharSequence generatePrintC(final Unit unit, final List<Row> rows, final boolean withDate) {
		...
		_builder.append("}");
		_builder.newLine();
		final String _generatePrint = this.generatePrint(rows);
	...
	public CharSequence generatePrintH(final Unit unit, final List<Row> rows, final boolean withDate) {
		...
		_builder.newLine();
		_builder.newLine();
		final String _generatePrintDef = this.generatePrintDef(rows);
	...
	public CharSequence generateAccessH(final Unit unit, final List<Row> rows, final boolean withDate) {
		...
		_builder.newLine();
		_builder.newLine();
		final String _generateDeclarations = this.generateDeclarations(unit, rows);
				...
				if ((ne instanceof Column)) {
					...
					_builder_1.newLineIfNotEmpty();
					{
						for (final NamedElement neRow : col.rows) {
							...
							_builder_1.newLineIfNotEmpty();
							_builder_1.append("\t");
							final String _name_2 = neRow.getName();
							_builder_1.append(_name_2, "\t");
							_builder_1.append("_t ");
							final String _name_3 = neRow.getName();
	...
	public CharSequence generateAccessC(final List<Row> rows, final boolean withDate) {
		...
		_builder.newLine();
		final String _generateSetterFunctions = this.generateSetterFunctions(rows);
		_builder.append(_generateSetterFunctions, "");
		_builder.newLineIfNotEmpty();
		_builder.newLine();
		_builder.append("//Getter functions");
		_builder.newLine();
		final String _generateGetterFunctions = this.generateGetterFunctions(rows);
		...
		for (final Row row : rows) {
			{
				...
				if ((!handled)) {
					final CharSequence _generateGetterFunction = this.generateGetterFunction(row, rows);
	...
	public CharSequence generateGetterFunction(final Row row, final List<Row> rows) {
		...
		_builder.newLine();
		_builder.append("\t");
		final String _generateAddressReadSwitch = this.generateAddressReadSwitch(row, rows);
		...
		for (final Row row : rows) {
			{
				...
				if (_and) {
					final CharSequence _generateSetterFunction = this.generateSetterFunction(row, rows);
	...
	public CharSequence generateSetterFunction(final Row row, final List<Row> rows) {
		...
		_builder.newLineIfNotEmpty();
		{
			...
			for (final Definition ne : _writeDefs_1) {
				_builder.append("\t");
				final CharSequence _generateConditions = this.generateConditions(row, ne);
	...
	public StringBuilder explain(final Definition d) {
		...
		if (_switchValue != null) {
			...
			case mask:
				...
				final long _doubleLessThan = (1l << d.width);
				final long _minus = (_doubleLessThan - 1);
			...
			case silentMask:
				...
				final long _doubleLessThan_1 = (1l << d.width);
				final long _minus_1 = (_doubleLessThan_1 - 1);
	...
	public CharSequence humanRange(final Definition d) {
		final StringConcatenation _builder = new StringConcatenation();
		{
			final boolean _tripleEquals = (d.type == Definition.Type.INT);
			if (_tripleEquals) {
	...
	public boolean hasWrite(final NamedElement ne) {
		boolean _and = false;
		final boolean _tripleNotEquals = (((Definition) ne).rw != Definition.RWType.r);
		if (!_tripleNotEquals) {
			_and = false;
		} else {
			final boolean _tripleNotEquals_1 = (((Definition) ne).type != Definition.Type.UNUSED);
	...
	public String getMaxValueHex(final Definition d) {
		final int _maxValue = this.getMaxValue(d);
		final String _hexString = Integer.toHexString(_maxValue);
		return ("0x" + _hexString);
	}

	public String getMaxValueNegHex(final Definition d) {
		final int _maxValue = this.getMaxValue(d);
		final int _plus = (_maxValue + 1);
		final String _hexString = Integer.toHexString(_plus);
		return ("-0x" + _hexString);
	}
