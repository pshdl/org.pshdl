...
/**
 * This class allows to attempt to resolve a {@link BigInteger} value for any
 * {@link IHDLObject}. Of course this only works when the given IHDLObject is
 * truly constant. Parameters are not considered constant, unless they can be
 * found in the given {@link HDLEvaluationContext}.
 *
...
public class ConstantEvaluate {
	...
	protected Optional<BigInteger> _constantEvaluate(final HDLVariableRef obj, final HDLEvaluationContext context, final Set<HDLQualifiedName> evaled) {
		...
		final boolean _isPresent_1 = hVar_1.isPresent();
		...
		if (_equals_1) {
			...
			final boolean _isPresent_2 = subEval.isPresent();
			if (_isPresent_2) {
				evaled.remove(fqn);
			}
			return subEval;
		...
		if (_equals_2) {
			...
			final boolean _isPresent_3 = cRefEval.isPresent();
