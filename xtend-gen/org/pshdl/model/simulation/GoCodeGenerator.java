...
public class GoCodeGenerator extends CommonCodeGenerator implements ITypeOuptutProvider {
	...
	public GoCodeGenerator(final ExecutableModel em, final int maxCosts, final String pkg, final String unit) {
		...
		this.pkg = pkg;
		final String _firstUpper = StringExtensions.toFirstUpper(unit);
		this.unit = _firstUpper;
		final CommonCompilerExtension _commonCompilerExtension = new CommonCompilerExtension(em, 64);
	...
	public IHDLInterpreterFactory<NativeRunner> createInterpreter(final File tempDir) {
		try {
			IHDLInterpreterFactory<NativeRunner> _xblockexpression = null;
			{
				...
				final ProcessBuilder _processBuilder = new ProcessBuilder("/usr/local/go/bin/go", "build", _absolutePath, _absolutePath_1);
	...
	protected CharSequence fieldType(final VariableInformation varInfo, final EnumSet<CommonCodeGenerator.Attributes> attributes) {
		boolean _or = false;
		final boolean _isNullOrEmpty = IterableExtensions.isNullOrEmpty(((Iterable<?>) Conversions.doWrapArray(varInfo.dimensions)));
		if (_isNullOrEmpty) {
			_or = true;
		} else {
	...
	protected CharSequence footer() {
		...
		_builder.newLine();
		{
			for (final VariableInformation vi : this.em.variables) {
				...
				final int _varIdx = this.getVarIdx(vi);
	...
	protected CharSequence singleOp(final Frame.FastInstruction fi, final String op, final int targetSizeWithType, final int pos, final int a,
		...
		{
			final boolean _tripleEquals = (fi.inst == Instruction.bit_neg);
			if (_tripleEquals) {
	...
	protected CharSequence twoOp(final Frame.FastInstruction fi, final String op, final int targetSizeWithType, final int pos, final int leftOperand, final int rightOperand,
		...
		CharSequence _xblockexpression = null;
		{
			...
			final boolean _tripleEquals_1 = (fi.inst == Instruction.sra);
			if (_tripleEquals_1) {
			...
			final boolean _tripleEquals_2 = (fi.inst == Instruction.sll);
			if (_tripleEquals_2) {
