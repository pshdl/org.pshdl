...
public class JavaCodeGenerator extends CommonCodeGenerator implements ITypeOuptutProvider {
	...
	protected CharSequence header() {
		final StringConcatenation _builder = new StringConcatenation();
		{
			final boolean _tripleNotEquals = (this.packageName != null);
			if (_tripleNotEquals) {
			...
			if (this.hasClock) {
				...
				_builder.newLineIfNotEmpty();
				{
					...
					for (final VariableInformation v_1 : _excludeNull_1) {
						...
						_builder.append("\", ");
						final int _varIdx_1 = this.getVarIdx(v_1, this.purgeAliases);
	...
	public CharSequence createChangeAdapter(final boolean useInterface) {
		...
		_builder.newLineIfNotEmpty();
		_builder.append("\t");
		final CharSequence _fieldDeclarations = this.fieldDeclarations(false, true);
