...
public class VerilogCodeGenerator extends CommonCodeGenerator {
	private final Multimap<Integer, Frame> writes = LinkedListMultimap.<Integer, Frame> create();

	private final Multimap<Integer, VariableInformation> posClocks = LinkedHashMultimap.<Integer, VariableInformation> create();

	private final Multimap<Integer, VariableInformation> negClocks = LinkedHashMultimap.<Integer, VariableInformation> create();
	...
	public String generateMainCode() {
		...
		_builder.newLine();
		{
			for (final VariableInformation vi_1 : this.em.variables) {
				final String _declaration = this.declaration(vi_1, CommonCodeGenerator.NONE);
				_builder.append(_declaration, "");
				_builder.newLineIfNotEmpty();
				{
					if (vi_1.isRegister) {
						final String _declaration_1 = this.declaration(vi_1, CommonCodeGenerator.SHADOWREG);
		...
		_builder.newLine();
		{
			final Set<Integer> _keySet = this.posClocks.keySet();
			for (final Integer ii : _keySet) {
				_builder.append("always @(posedge ");
				final InternalInformation _get = this.em.internals[(ii).intValue()];
				final CharSequence _idName_2 = this.idName(_get.info, true, CommonCodeGenerator.NONE);
				...
				_builder.newLine();
				{
					final Collection<VariableInformation> _get_1 = this.posClocks.get(ii);
					for (final VariableInformation vi_2 : _get_1) {
						_builder.append("\t");
						final CharSequence _idName_3 = this.idName(vi_2, true, CommonCodeGenerator.NONE);
						_builder.append(_idName_3, "\t");
						_builder.append(" <= ");
						final CharSequence _idName_4 = this.idName(vi_2, true, CommonCodeGenerator.SHADOWREG);
		...
		_builder.newLine();
		{
			final Set<Integer> _keySet_1 = this.negClocks.keySet();
			for (final Integer ii_1 : _keySet_1) {
				_builder.append("always @(negedge ");
				final InternalInformation _get_2 = this.em.internals[(ii_1).intValue()];
				final CharSequence _idName_5 = this.idName(_get_2.info, true, CommonCodeGenerator.NONE);
				...
				_builder.newLine();
				{
					final Collection<VariableInformation> _get_3 = this.negClocks.get(ii_1);
					for (final VariableInformation vi_3 : _get_3) {
						_builder.append("\t");
						final CharSequence _idName_6 = this.idName(vi_3, true, CommonCodeGenerator.NONE);
						_builder.append(_idName_6, "\t");
						_builder.append(" <= ");
						final CharSequence _idName_7 = this.idName(vi_3, true, CommonCodeGenerator.SHADOWREG);
		...
		_builder.newLine();
		{
			final Set<Integer> _keySet_2 = this.writes.keySet();
			for (final Integer ii_2 : _keySet_2) {
				final CharSequence _generateWriteProcess = this.generateWriteProcess(ii_2);
	...
	public CharSequence generateWriteProcess(final Integer internal) {
		...
		final Collection<Frame> _get = this.writes.get(internal);
		final Function1<Frame, Set<CharSequence>> _function = new Function1<Frame, Set<CharSequence>>() {
			...
			public Set<CharSequence> apply(final Frame it) {
				...
				final List<CharSequence> _map = ListExtensions.<Integer, CharSequence> map(((List<Integer>) Conversions.doWrapArray(it.internalDependencies)), _function);
				return IterableExtensions.<CharSequence> toSet(_map);
			}
		};
		final Iterable<Set<CharSequence>> _map = IterableExtensions.<Frame, Set<CharSequence>> map(_get, _function);
		final Iterable<CharSequence> _flatten = Iterables.<CharSequence> concat(_map);
		final String _join = IterableExtensions.join(_flatten, " or ");
		...
		_builder.newLine();
		{
			final Collection<Frame> _get_1 = this.writes.get(internal);
			for (final Frame f : _get_1) {
				_builder.append("\t");
				final Function1<Integer, String> _function_1 = new Function1<Integer, String>() {
					@Override
					public String apply(final Integer it) {
						final CharSequence _sensitiviyName = VerilogCodeGenerator.this.sensitiviyName(it);
						return (_sensitiviyName + " == 0");
					}
				};
				final List<String> _map_1 = ListExtensions.<Integer, String> map(((List<Integer>) Conversions.doWrapArray(f.predNegDepRes)), _function_1);
				...
				final List<CharSequence> _map_2 = ListExtensions.<Integer, CharSequence> map(((List<Integer>) Conversions.doWrapArray(f.predPosDepRes)), _function_2);
				final Iterable<CharSequence> predSet = Iterables.<CharSequence> concat(_map_1, _map_2);
				_builder.newLineIfNotEmpty();
				{
					final boolean _isEmpty = IterableExtensions.isEmpty(predSet);
					final boolean _not = (!_isEmpty);
					if (_not) {
						_builder.append("\t");
						_builder.append("if (");
						final String _join_1 = IterableExtensions.join(predSet, " && ");
						...
						_builder.newLine();
						_builder.append("\t");
						final CharSequence _frameExecution = this.frameExecution(f);
						...
						_builder.append("\t");
						final CharSequence _frameExecution_1 = this.frameExecution(f);
	...
	protected StringBuilder toCastExpression(final Frame.FastInstruction exec, final Frame frame, final int pos, final int a, final int b, final int arrPos) {
		...
		if (_switchValue != null) {
			...
			case bitAccessSingle:
				...
				_builder.append("]");
				final StringBuilder _assignTempVar = this.assignTempVar(VariableInformation.Type.BIT, 2, pos, CommonCodeGenerator.NONE, _builder, false);
			...
			case bitAccessSingleRange:
				...
				_builder_1.append("]");
				final StringBuilder _assignTempVar_1 = this.assignTempVar(VariableInformation.Type.BIT, (targetSize << 1), pos, CommonCodeGenerator.NONE, _builder_1, false);
				sb.append(_assignTempVar_1);
				break;
			case cast_int:
				final int _min = Math.min(exec.arg1, exec.arg2);
				final int _doubleLessThan = (_min << 1);
				final int _bitwiseOr = (_doubleLessThan | 1);
				final StringConcatenation _builder_2 = new StringConcatenation();
				_builder_2.append(tempName, "");
				_builder_2.append("[");
				final int _min_1 = Math.min(exec.arg1, exec.arg2);
				final int _minus = (_min_1 - 1);
				_builder_2.append(_minus, "");
				_builder_2.append(":0]");
				final StringBuilder _assignTempVar_2 = this.assignTempVar(VariableInformation.Type.INT, _bitwiseOr, pos, CommonCodeGenerator.NONE, _builder_2, false);
				sb.append(_assignTempVar_2);
				break;
			case cast_uint:
				final int _min_2 = Math.min(exec.arg1, exec.arg2);
				final int _doubleLessThan_1 = (_min_2 << 1);
				final StringConcatenation _builder_3 = new StringConcatenation();
				_builder_3.append(tempName, "");
				_builder_3.append("[");
				final int _min_3 = Math.min(exec.arg1, exec.arg2);
				final int _minus_1 = (_min_3 - 1);
				_builder_3.append(_minus_1, "");
				_builder_3.append(":0]");
				final StringBuilder _assignTempVar_3 = this.assignTempVar(VariableInformation.Type.UINT, _doubleLessThan_1, pos, CommonCodeGenerator.NONE, _builder_3, false);
	...
	public CharSequence sensitiviyName(final Integer internal) {
		final InternalInformation _get = this.em.internals[(internal).intValue()];
	...
	public String declaration(final VariableInformation vi, final EnumSet<CommonCodeGenerator.Attributes> attributes) {
		...
		final boolean _contains = attributes.contains(CommonCodeGenerator.Attributes.isShadowReg);
		if (_contains) {
			type = "reg";
		}
		final StringConcatenation _builder = new StringConcatenation();
		...
		final boolean _isArray_1 = this.isArray(vi);
		if (_isArray_1) {
			if ((vi.width != 1)) {
				final StringConcatenation _builder_3 = new StringConcatenation();
				_builder_3.append("[0:");
				final int _arraySize_1 = this.getArraySize(vi);
				final int _minus_1 = (_arraySize_1 - 1);
				_builder_3.append(_minus_1, "");
				_builder_3.append("]");
				dim = _builder_3;
			}
		...
		_builder_4.append(" ");
		final CharSequence _idName = this.idName(vi, true, attributes);
