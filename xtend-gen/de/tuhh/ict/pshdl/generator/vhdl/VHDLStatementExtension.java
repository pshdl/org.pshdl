...
public class VHDLStatementExtension {
	...
	private static GenericMeta<HDLQualifiedName> ORIGINAL_FULLNAME = new Function0<GenericMeta<HDLQualifiedName>>() {
		...
		public GenericMeta<HDLQualifiedName> apply() {
			GenericMeta<HDLQualifiedName> _genericMeta = new GenericMeta<HDLQualifiedName>("ORIGINAL_FULLNAME", true);
			return _genericMeta;
		}
	}.apply();
	...
	protected VHDLContext _toVHDL(final HDLBlock obj, final int pid) {
		...
		if (!_notEquals) {
			...
			_and = (_notEquals && (_process_1).booleanValue());
		}
		if (_and) {
			process = true;
		}
			...
			for (final HDLExpression exp : _dimensions_1) {
				{
					...
					HDLArithOp _setType = _setLeft.setType(HDLArithOpType.MINUS);
			...
			for (final HDLVariable hvar : _variables) {
				{
					...
					if (_notEquals_4) {
						...
						ConstrainedArray _constrainedArray = new ConstrainedArray(_arrayRefName, type, ((List<DiscreteRange>) Conversions.doWrapArray(arrRangs)));
