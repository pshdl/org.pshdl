...
public class BuiltInAdvisor {
	...
	public static HDLAdvise advise(Problem problem) {
		...
		case ANNOTATION_INVALID:
			...
			return new HDLAdvise(
					problem,
					"The String you provided is incorrect for this annotation: "
							+ problem.info,
					...
					"Check the message for information what might be incorrect",
					"Check the documentation for the " + annoName
							+ " Annotation");
		case ANNOTATION_UNKNOWN: {
			final String lastName = ((HDLAnnotation) problem.node).getName();
			final Set<String> knownAnnotations = HDLCore
					.getCompilerInformation().registeredAnnotations.keySet();
			final String annoProposal = getMatchProposal(knownAnnotations,
					"Annotations", lastName);
			return new HDLAdvise(
					problem,
					"The annotation: " + lastName + " is not known",
					"The annotation you have used is not known to the compiler. Maybe it was misspelled or it is not installed.",
					annoProposal,
		...
		case ARRAY_INDEX_NEGATIVE:
			return new HDLAdvise(
					problem,
					"The index of the array can only be negative",
					"When an array is accessed, the index has to be positive as negative indexes don't make sense",
					"Cast the index to uint");
		case ARRAY_INDEX_OUT_OF_BOUNDS: {
			final Range<BigInteger> accessRange = problem
					.getMeta(BuiltInValidator.ACCESS_RANGE);
			final Range<BigInteger> arrayRange = problem
					.getMeta(BuiltInValidator.ARRAY_RANGE);
			return new HDLAdvise(
					problem,
					"The array index exceeds its capacity",
					"Valid access index for the array are: "
							+ arrayRange
							+ " while the index has a range of: "
							+ accessRange
							+ ". These don't overlap which means that the index will never be valid");
		}
		case ARRAY_INDEX_POSSIBLY_NEGATIVE: {
			...
			if (!arrayRange.isConnected(accessRange)) {
				solutions = new String[] { "Cast the index to uint with:(uint)"
						+ problem.node };
			} else {
				solutions = new String[] {
						"Cast the index to uint with:(uint)" + problem.node,
						"Manually declare a range for the index with the @range(\""
								+ arrayRange.lowerEndpoint() + ";"
								+ arrayRange.upperEndpoint()
								+ "\") annotation to define a range" };
			}
			return new HDLAdvise(
					problem,
					"The array index could possibly become negative",
					"The given array index has a possible negative value ("
							+ accessRange.lowerEndpoint()
							+ "), even tough it does not need to become negative by design, it would be possible. This moght indicate a programming error",
					solutions);
		}
		case ARRAY_INDEX_POSSIBLY_OUT_OF_BOUNDS: {
			...
			final Range<BigInteger> arrayRange = problem
					.getMeta(BuiltInValidator.ARRAY_RANGE);
			final Range<BigInteger> commonRange = arrayRange
					.intersection(accessRange);
			return new HDLAdvise(problem,
					"The array index can exceed its capacity",
					"The given array index has a possible range of:"
							+ accessRange
							+ " while the highest index of the array is "
							+ arrayRange.upperEndpoint(),
					"Limit the possible range by masking with &",
					"Downcast the index to a suitable size",
					"Use the @range(\"" + commonRange.lowerEndpoint() + ";"
							+ commonRange.upperEndpoint()
							+ "\") Annotation to indicate the expected range");
		}
		case ARRAY_REFERENCE_NOT_SAME_DIMENSIONS:
			return new HDLAdvise(
					problem,
					"The dimensions of assignment do not match",
					"When an array is assigned to another array, the size of the dimension need to match.",
					"Check that the right handside of the array has the same dimension(s) as the left handside");
		case BIT_ACCESS_NOT_POSSIBLE:
			return new HDLAdvise(
					problem,
					"Can not access bits of this type",
					"Only bits from primitive types with a known with can be access, that is either int<?>,uint<?>,bit<?>.",
					"cast type to a known width");
		case COMBINED_ASSIGNMENT_NOT_ALLOWED:
			return new HDLAdvise(
					problem,
					"A combined assignment can only be used on registers",
		...
		case CONSTANT_DEFAULT_VALUE_NOT_CONSTANT:
			return new HDLAdvise(
					problem,
					"The value of a constant needs to be constant",
					"You can not use parameters or other variables for computing the value of a constant",
					"Ensure that only constant variables are used as value");
		case CONSTANT_NEED_DEFAULTVALUE:
			return new HDLAdvise(problem, "Constants need to have a value",
					"A constant without value is pretty pointless",
					"Assign a default value to the constant");
		case EQUALITY_ALWAYS_FALSE:
			return new HDLAdvise(
					problem,
					"This equality is always false.",
					"This behaviour might be desired, but it might also indicate a programming error.");
		...
		case GENERATOR_ERROR:
			return new HDLAdvise(problem, "The generator contains an error",
					problem.info,
					"Read the documentation for the specific generator");
		case GENERATOR_WARNING:
			return new HDLAdvise(problem, "The generator produced a warning",
					problem.info,
					"Read the documentation for the specific generator");
		case GENERATOR_INFO:
			return new HDLAdvise(problem,
					"The generator contains an information", problem.info);
		case GENERATOR_NOT_KNOWN: {
			final String genName = ((HDLDirectGeneration) problem.node)
					.getGeneratorID();
			final Set<String> genIDs = HDLCore.getCompilerInformation().registeredGenerators
					.keySet();
			final String generatorProposal = getMatchProposal(genIDs,
					"Generators", genName);
			return new HDLAdvise(
					problem,
					"The generator with the id: " + genName + " is not known",
					"The generator you have used is not known to the compiler. Maybe it was misspelled or it is not installed.",
					generatorProposal,
		...
		case INTERFACE_IN_PORT_NEVER_WRITTEN: {
			...
			return new HDLAdvise(
					problem,
					"No write access to the in port: " + var.getName()
							+ " of the instance: " + hii.getVar().getName()
							+ " detected",
					...
					"Write a meaningful value to the port",
					"Write a zero to it: " + hii.getVar().getName() + "."
							+ var.getName() + " = 0;");
		}
		case INTERFACE_OUT_PORT_NEVER_READ: {
			...
			return new HDLAdvise(
					problem,
					"No read access to the out port: " + var.getName()
							+ " of the instance: " + hii.getVar().getName()
							+ " detected",
					"It appears that the port "
							+ var.getName()
							+ " is never read, altough it is marked as out port",
					"Do something with the port");
		}
		case INTERFACE_OUT_WRITTEN: {
			final HDLVariable var = (HDLVariable) problem.context;
			final HDLInterfaceInstantiation hii = (HDLInterfaceInstantiation) problem.node;
			return new HDLAdvise(problem, "The out port: " + var.getName()
					+ " of the instance: " + hii.getVar().getName()
					+ " is written", "It appears that the port "
					+ var.getName()
					+ " is written to, altough it is marked as out port",
		...
		case INTERFACE_UNUSED_PORT: {
			...
			return new HDLAdvise(
					problem,
					"The port: " + var.getName() + " of the instance: "
							+ hii.getVar().getName()
							+ " is never read or written",
					"It appears that the port "
							+ var.getName()
							+ " is neither read, nor written to. You might want to check wether this is intentional",
					"Remove the port if it is not necessary",
		...
		case INTERNAL_SIGNAL_READ_BUT_NEVER_WRITTEN: {
			...
			return new HDLAdvise(
					...
					"Only reading an internal variable does not make much sense as it needs to be written as well",
					"Write to it", "Remove it");
		}
		case INTERNAL_SIGNAL_WRITTEN_BUT_NEVER_READ: {
			...
			return new HDLAdvise(
					...
					"Only writing an internal variable does not make much sense as it needs to be read as well",
					"Read from it", "Remove it");
		}
		case IN_PORT_CANT_REGISTER:
			return new HDLAdvise(
					problem,
					"A variable of direction in can not be declared a register",
					"It does not make much sense to declare a incoming variable as register as the data is directly coming from the outside",
					"Revmoed the register keyword");
		case IN_PORT_NEVER_READ: {
			final HDLVariable var = (HDLVariable) problem.node;
			return new HDLAdvise(
					problem,
					"The variable:" + var.getName()
							+ " is declared with direction in, but never read",
					"A variable with direction in should be used otherwise it is rather useless",
					"Remove it");
		}
		case MULTI_PROCESS_WRITE: {
			return new HDLAdvise(
					problem,
					"Only a single process can write to a variable",
		...
		case NO_SUCH_FUNCTION: {
			final HDLQualifiedName genName = ((HDLFunctionCall) problem.node)
					.getNameRefName();
			return new HDLAdvise(
					problem,
					"The function:" + genName + " can not be found",
					"The function could not be found, maybe you misspelled it.",
					"Double check the name", "Check your imports");
		}
		case ONLY_ONE_CLOCK_ANNOTATION_ALLOWED:
			return new HDLAdvise(
					problem,
					"Only one variable is allowed to be annotated with the @clock annotation",
		...
		case ONLY_ONE_RESET_ANNOTATION_ALLOWED:
			return new HDLAdvise(
					problem,
					"Only one variable is allowed to be annotated with the @reset annotation",
		...
		case PARAMETER_OR_CONSTANT_NEVER_READ: {
			...
			return new HDLAdvise(
					problem,
					"The constant:" + var.getName() + " is never read",
					"Declaring constants and not reading them is not very useful.",
					"Use it", "Remove it");
		}
		case UNRESOLVED_ENUM: {
			return new HDLAdvise(
					problem,
					"The enum:" + problem.node + " can not be resolved",
					"Either the referenced enum does not declare an enum with that name, or the referenced enum could not be found.",
					"Check that the name is correct",
					"Check that the type is imported or fully referenced");
		}
		case UNRESOLVED_INTERFACE:
			return new HDLAdvise(problem, "The interface:" + problem.node
					+ " can not be resolved",
					"An interface with that name can not be found",
					"Check that the name is correct",
					"Check that the type is imported or fully referenced");
		case UNRESOLVED_REFERENCE:
			return new HDLAdvise(problem, "The reference:" + problem.node
					+ " can not be resolved",
					"A type/function/variable with that name can not be found",
					"Check that the name is correct",
					"Check that the type is imported or fully referenced");
		case UNRESOLVED_TYPE:
			return new HDLAdvise(problem, "The type:" + problem.node
					+ " can not be resolved",
					"A type with that name can not be found",
					"Check that the name is correct",
					"Check that the type is imported or fully referenced");
		case UNRESOLVED_VARIABLE:
			return new HDLAdvise(
					problem,
					"The variable:" + problem.node + " can not be resolved",
					"A variable with that name can not be found",
					"Check that the name is correct",
		...
		case UNRESOLVED_FRAGMENT: {
			...
			if (uf instanceof HDLUnresolvedFragmentFunction) {
				final Set<String> funcIDs = HDLCore.getCompilerInformation().registeredFunctions
						.keySet();
				similiarProposal = getMatchProposal(funcIDs, "Functions",
						uf.getFrag());
			} else {
				final Set<String> vars = collectVariables(uf);
				similiarProposal = getMatchProposal(vars, "Variables",
						uf.getFrag());
			}
			return new HDLAdvise(
					problem,
					...
					"If you want to reference a variable check that the spelling is correct and that the variable is visible in this scope",
					similiarProposal);
		}
		case UNRESOLVED_FUNCTION:
			...
			final Set<String> funcIDs = HDLCore.getCompilerInformation().registeredFunctions
					.keySet();
			final String functionProposal = getMatchProposal(funcIDs,
					"Functions", fc.getNameRefName().toString());
			return new HDLAdvise(problem, "The function:" + problem.node
					+ " can not be resolved",
					"A function with that name can not be found",
					functionProposal,
		...
		case UNSUPPORTED_TYPE_FOR_OP:
			...
			case HDLArithOp:
				return new HDLAdvise(
						problem,
						problem.info,
						"Arithmetic operations require an interpretable value, they can not work on bit types",
						"Cast either side to uint<?>/int<?>");
			case HDLShiftOp:
				return new HDLAdvise(
						problem,
						problem.info,
						"Shift operations require an interpretable value on the right hand side, they can not work on bit types",
			...
			case HDLBitOp:
				return new HDLAdvise(
						problem,
						problem.info,
						"Bit operations generally work on any primitive type, see the message for more information");
			default:
		...
		case UNUSED_VARIABLE:
			return new HDLAdvise(
					problem,
					"The variable: " + problem.node + " is not used",
					"This variable is never read or written. You can safely remove it",
					"Remove it", "Use it");
		case WRITE_ACCESS_TO_IN_PORT:
			return new HDLAdvise(
					problem,
					"The variable: " + problem.node
							+ " is defined as in port and written",
					"A variable that is declared as in can only be read. Writing to it is not supported",
					"Remove the in keyword in the declaration",
					"Assign it to a new internal variable",
					"Change the direction to inout");
		case ARRAY_REFERENCE_TOO_MANY_DIMENSIONS:
			return new HDLAdvise(
					problem,
					"The reference contains more array dimensions than the declared type",
					"A variable that is declared with just one dimension, can not be access with two or more",
					"Check that the declared type has the expected number of dimensione",
					"Remove one of more array acceses");
		case ASSIGNMENT_NOT_SUPPORTED:
			return new HDLAdvise(problem, problem.info,
					"Some types can not be automatically converted.",
					"Convert the type explicitly",
					"Check that the assignment target has the expected type");
		case ASSIGNMENT_CLIPPING_WILL_OCCUR:
			return new HDLAdvise(
					problem,
					"The assigned target has fewer bits than the assigned value",
					"The assigned value will be truncated",
					"Widen the assignment target",
					"Explicitly down cast the assigned value");
		case ASSIGNMENT_NOT_ENUM:
			return new HDLAdvise(problem, "Assigned value is not an enum",
					"Enums can only accept the same type of enum.",
					"Assign a proper enum");
		...
		case CONCAT_TYPE_NOT_ALLOWED:
			return new HDLAdvise(
					problem,
					"Only variables with known width are allowed in concatenations",
		...
		case SWITCH_LABEL_DUPLICATE:
			return new HDLAdvise(
					...
					"Each case within a switch must have a unique value, duplicates are not allowed",
					"Remove or merge the offending case");
		case SWITCH_LABEL_NOT_CONSTANT:
			return new HDLAdvise(problem,
					"Switch cases need to have a constant value", "",
					"Ensure that the used case value is constant");
		case SWITCH_MULTIPLE_DEFAULT:
			return new HDLAdvise(problem,
					"Switch case does not contain more than one default case",
					"All switch statements require exactly one default case.",
					"Remove one default case");
		case SWITCH_NO_DEFAULT:
			return new HDLAdvise(
					problem,
					"Switch case does not contain a default case",
					"All switch statements require a default case. This is to avoid missed cases which lead to an incomplete description",
					"Add a default case");
		case VARIABLE_KEYWORD_NAME:
			return new HDLAdvise(
					problem,
					"The variable name is a keyword in PSHDL",
					"Keywords can not be used as variable names as the parser would fail",
					"Rename the variable");
		case VARIABLE_SAME_NAME:
			return new HDLAdvise(
					problem,
					"A variable with this name already exists",
					"The compiler can not tell those variables apart as they have the exact same name",
		...
		case VARIABLE_SAME_NAME_DIFFERENT_CASE: {
			...
			return new HDLAdvise(
					problem,
					"The variable " + var.getName()
							+ " already exists in a slightly different case:"
							+ other.getName(),
					"When compiling to VHDL, a case insensitive language, a name collision will occour",
					"Rename either variable");
		}
		case VARIABLE_SCOPE_SAME_NAME:
			return new HDLAdvise(
					problem,
					"A variable with this name already exists",
					"In a scope above this scope a similar named variable is already declared",
					"Rename either variable");
		...
		case INLINE_FUNCTION_NO_TYPE:
			return new HDLAdvise(
					problem,
					"Can not determine (return) type of: " + problem.node,
					"This message should not be displayed to a user, so if you see this, please write a bug report about it",
					"File a bug report");
		case ARRAY_INDEX_NO_RANGE:
			return new HDLAdvise(
					problem,
					"The possible value range used for accessing the array can not be determined",
		...
		case BIT_ACCESS_NOT_POSSIBLE_ON_TYPE:
			return new HDLAdvise(
					problem,
					"This type does not support bit acess",
					"Only primitive types like int.uint and bit of any width allow access to their bits",
					"Check that you are accesssing the correct variable");
		case CONSTANT_PORT_CANT_REGISTER:
			return new HDLAdvise(
					problem,
					"Constants can not be a register",
					"A constant can not be of type register as this implies that it could be modified",
					"Remove the register keyword");
		case GLOBAL_CANT_REGISTER:
			return new HDLAdvise(problem,
					"A global variable can not be a register",
					"Global variables can only be constant",
		...
		case RANGE_NOT_DOWN:
			return new HDLAdvise(
					...
					"Annotate either index with a '@range' annotation to help the compiler");
		case RANGE_NOT_UP:
			return new HDLAdvise(
					problem,
					"Range not of direction up",
		...
		case RANGE_OVERLAP:
			return new HDLAdvise(
					...
					"Convert ranges of type {A+x:A} to {A+:x}",
					"Convert ranges of type {A:A-x} to {A-:x}",
					"Annotate the variable with @range to guarantee a certain input range");
		case UNKNOWN_RANGE:
			return new HDLAdvise(
					problem,
					"Can not determine direction of range",
		...
		case GLOBAL_NOT_CONSTANT:
			return new HDLAdvise(problem, "Constants have to be constant",
					"The default value of this global constant is not constant");
		case FUNCTION_SAME_NAME:
			return new HDLAdvise(problem,
					"A function with this name already exists",
					"Function names have to be unique",
					"Rename this function or the other");
		case GLOBAL_VAR_SAME_NAME:
			return new HDLAdvise(problem,
					"A global variable with this name already exists",
					"Global variables have to have a unique name",
					"Rename this variable or the other");
		case TYPE_SAME_NAME:
			return new HDLAdvise(problem,
					"A type with this name already exists",
					"Type names have to be unique",
					"Rename this type or the other");
		...
		case VARIABLE_NAME_NOT_RECOMMENDED:
			return new HDLAdvise(
					problem,
					"Variables starting with $ are not recommended",
	...
	private static String getMatchProposal(Set<String> possibleNames,
			String type, String name) {
		final Score[] topMatches = LevenshteinDistance.getTopMatches(name,
				true, possibleNames.toArray(new String[possibleNames.size()]));
	...
	private static Set<String> collectVariables(IHDLObject obj) {
		...
		if (unit != null) {
			...
			final List<HDLVariable> vars = ScopingExtension.INST
					.doGetVariables(unit);
